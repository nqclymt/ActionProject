...
namespace PKC.ActionEditor
{
    public static class IDirectableExtensions
    {
        ...
        public static IDirectable GetPreviousSibling(this IDirectable directable, IDirectable parent)
        {
            if (parent != null)
            {
                return parent.Children.Where(d => d != directable && d.StartTime < directable.StartTime)
        ...
        public static float GetPreviousLoopLocalTime(this ISubClipContainable clip)
        {
            ...
            if (Mathf.Abs(clip.SubClipSpeed) <= Mathf.Epsilon || clip.SubClipLength <= Mathf.Epsilon)
            {
                return clipLength;
            }
